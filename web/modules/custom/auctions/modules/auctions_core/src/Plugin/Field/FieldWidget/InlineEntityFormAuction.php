 ...
 *
 * @FieldWidget(
 *   id = "inline_entity_form_auction",
 *   label = @Translation("Inline entity form - Auction"),
 *   field_types = {
 *     "entity_reference",
 ...
 * )
 */
class InlineEntityFormAuction extends InlineEntityFormBase implements ContainerFactoryPluginInterface {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    foreach ($entities as $key => $value) {
      ...
      if (!empty($value['form'])) {
        ...
        // @todo relist button on IFE from
        //   if (empty($entity_id) || $entity->access('update')) {
        // $row['actions']['ief_entity_relist'] = [
        // '#type' => 'submit',
        // '#value' => $this->t('relist'),
        // '#name' => 'ief-' . $this->getIefId() . '-entity-relist-' . $key,
        // '#limit_validation_errors' => [],
        // '#ajax' => [
        // 'callback' => 'inline_entity_form_get_element',
        // 'wrapper' => $wrapper,
    ...
    $open_form = $form_state->get(['inline_entity_form', $this->getIefId(),
      'form',
    ]);
      ...
      elseif ($widget_state['form'] == 'ief_add_existing') {
        ...
        if (!empty($element['#value'])) {
          ...
          foreach ($input_values as $input) {
            ...
            if ($match === NULL) {
              // Try to get a match from the input string when the user dont use
   ...
   */
  public static function submitCloseRow($form, $form_state) {
  ...
  public static function updateRowWeights($element, FormStateInterface $form_state, $form) {
    ...
    // Loop over the submitted delta values and update the weight of entities
    // in the form state.
  ...
  /**
   * Add common submit callback functions and mark the element as IEF trigger.
   *
   * @param array $element
   *   The element to which the submit callbacks should be added.
   */
